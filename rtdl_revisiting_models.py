...
class FTTransformer(nn.Module):
    ...
    def __init__(
        ...
        **backbone_kwargs,
        ...
        print(backbone_kwargs) # 'n_blocks': 3, 'd_block': 192
        ...
        # ================================== KAFT ==========================================
        # from kan.efficient_kan import KANEmbeddings
        # self.cont_embeddings = (
        #     KANEmbeddings(
    ...
    def make_default_optimizer(self) -> torch.optim.AdamW:
        ...
        return torch.optim.AdamW(
            self.make_parameter_groups(), lr=1e-3, weight_decay=1e-5
        )
    ...
    def forward(self, x_cont: Tensor) -> Tensor:
        """Do the forward pass."""
        if x_cont is None:
            raise ValueError('x_cont must be provided.')

        x_embeddings: List[Tensor] = []
        if self.cls_embedding is not None:
            x_embeddings.append(self.cls_embedding(x_cont.shape[:-1]))
...
if __name__ == "__main__":
    from kan.efficient_kan import KANEmbeddings
    # 设置输入参数
    n_features = 5  # 连续特征的数量
    d_embedding = 8  # 嵌入维度
    batch_size = 3  # 批量大小

    # 初始化 LinearEmbeddings 模块,输入形状: torch.Size([bs, n_features]), 输出形状: torch.Size([bs, n_features, d_embedding])
    linear_embeddings = LinearEmbeddings(n_features=n_features, d_embedding=d_embedding)

    # 初始化 KANLinear 模块
    kan_embeddings = KANEmbeddings(
    ...
    # 创建随机输入数据
    x = torch.randn(batch_size, n_features)  # 输入形状为 (batch_size, n_features)
    print("输入数据 (x):")
    print(x)
    print(f"输入形状: {x.shape}\n")

    # 前向传播
    # output = linear_embeddings(x)
    output = kan_embeddings(x)
    print("输出数据 (output):")
    print(output)
