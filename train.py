...
def evaluate_by_snr(
    ...
    test_x = data["test"]["x"]
    test_y = data["test"]["y"]  # Keep as tensor, don't convert to numpy
    ...
    with autocast('cuda'):
        outputs = []
        # Create a dictionary with tensors (not numpy arrays)
        test_data = {"x": test_x, "y": test_y}
        for batch_cpu in delu.iter_batches(test_data, batch_size=batch_size, shuffle=False):
            batch = {k: v.to(device) for k, v in batch_cpu.items()}
            output = apply_model(model, batch)
            outputs.append(output.cpu().numpy())
        y_pred_np = np.concatenate(outputs, axis=0)
    ...
    y_pred_classes = np.argmax(y_pred_np, axis=1)
    test_y_classes = test_y.cpu().numpy()  # Convert to numpy for comparison
