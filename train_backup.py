...
def define_model(n_cont_features: int, n_classes: int, lr: float):
    """定义模型"""
    model = FTTransformer(
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)  # 使用传入的学习率
    # 添加学习率调度器
...
def train_model(model, optimizer, scheduler, data, n_epochs, patience, batch_size, accumulation_steps=1, lr=1e-3):
    """训练模型"""
    loss_fn = F.binary_cross_entropy_with_logits  # 固定使用多分类损失函数
    scaler = GradScaler()
...
def evaluate(model, data, part: str, batch_size=32) -> Tuple[float, float, float]:
    """评估模型并统计吞吐量和延迟
    Returns:
        score (float): 模型精度（如准确率）
        throughput (float): 吞吐量（样本/秒）
        latency (float): 平均延迟（秒/样本）
    """
    model.eval()
...
def evaluate_by_snr(
    ...
    for snr in snrs:
        ...
        true_snr = y_true_classes[indices_of_snr]
        if len(pred_snr) == 0:
...
def main():
    ...
    model, optimizer, scheduler = define_model(n_cont_features, n_classes, args.lr) 
